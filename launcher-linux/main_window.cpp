...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
  setWindowTitle(QStringLiteral("NFS Most Wanted — ARM64"));
  resize(920, 650);
  ...
  setStyleSheet(QStringLiteral(R"(
    QMainWindow, QWidget { background: #0a0e14; color: #e8edf5; }
    QTabWidget::pane, QGroupBox { border: 1px solid #3a4558; }
    QTabBar::tab { background: #141a24; padding: 10px 18px; }
    QTabBar::tab:selected { background: #c45c12; }
    QPushButton { background: #d96a16; border: 1px solid #f0a04a;
                  padding: 8px 14px; border-radius: 3px; color: #0a0e14;
                  font-weight: bold; }
    QPushButton:hover { background: #f07a20; }
    QPushButton:disabled { background: #2a3340; color: #7a8699;
                           border-color: #3a4558; }
    QComboBox, QSpinBox { background: #141a24; padding: 5px; }
  ...
  if (!dataRoot_.isEmpty()) {
    const QString testFile =
        QDir(dataRoot_).filePath(QStringLiteral(".__nfsmw_write_test__"));
    QFile f(testFile);
...
QWidget* MainWindow::createPlayPage() {
  ...
  auto* title = new QLabel(tr("NEED FOR SPEED\nMOST WANTED"));
  ...
  auto* version =
      new QLabel(tr("Launcher ARM64 %1").arg(QStringLiteral(NFSMW_LAUNCHER_VERSION)));
...
QWidget* MainWindow::createGraphicsPage() {
  ...
  auto* note = new QLabel(tr(
      ...
      "automatically for ARM64/Turnip. The Xenos GPU plugin is always "
      "loaded."));
...
QWidget* MainWindow::createLocalePage() {
  ...
  auto* note = new QLabel(tr(
      "This build currently expects the PAL Spain dump. Spanish uses "
      "Language ID 5 and Country ID 31. The game must contain files for "
...
QString MainWindow::gameExecutable() const {
  ...
  const QStringList candidates{
      app.filePath(QStringLiteral("nfsmw")),
      app.filePath(QStringLiteral("../bin/nfsmw")),
      app.filePath(QStringLiteral("../nfsmw")),
  };
...
void MainWindow::play() {
  ...
  if (executable.isEmpty()) {
    QMessageBox::critical(this, tr("Executable not found"),
                          tr("nfsmw was not found next to the launcher."));
    return;
  }
